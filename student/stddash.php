...
ini_set('display_errors', 1); // Change to 1 for development
...
// Fetch student data
$stdsql = "SELECT id, name, pass, email, photo FROM student";
$stdresult = $conn->query($stdsql);
$stdusers = [];

if ($stdresult && $stdresult->num_rows > 0) {
    while ($row = $stdresult->fetch_assoc()) {
        $stdusers[] = $row;
    }
    // Store specific student in session from login
    if (isset($_SESSION['user_id'])) {
        $specificUserId = $_SESSION['user_id'];
        foreach ($stdusers as $user) {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['add_user'])) {
    ...
    $stmt = $conn->prepare("INSERT INTO assignment (course_id, topic, lastdate, upload_date) VALUES (?, ?, ?, ?)");
    $stmt->bind_param("ssss", $course_id, $topic, $lastdate, $upload_date);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['noteid'])) {
    $noteid = $_POST['noteid']; // Ensure this is a valid note ID
    $user_id = $_SESSION['user_id'];
    $message = isset($_POST['message']) ? $_POST['message'] : null;
    $file_name = isset($_FILES['file']) ? $_FILES['file']['name'] : null;
    $file_type = isset($_FILES['file']) ? $_FILES['file']['type'] : null;
    $file_data = isset($_FILES['file']) ? file_get_contents($_FILES['file']['tmp_name']) : null;

    // Validate message and file
    if (empty($message) && empty($file_data)) {
        echo "Please provide a message or upload a file.";
    } else {
        // Generate a unique note ID if not provided
        if (empty($noteid)) {
            $noteid = uniqid('note_', true); // Prefix to ensure uniqueness
        }

        $stmt = $conn->prepare("INSERT INTO notes (noteid, user_id, message, file_name, file_type, file_data) VALUES (?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("ssssss", $noteid, $user_id, $message, $file_name, $file_type, $file_data);
        ...
        <div class="flex items-center space-x-4 w-full">
            ...
            <div class="flex items-center space-x-4">
                ...
                <div class="flex items-center">
                    ...
                    <button id="logoutBtn" class="ml-2 bg-red-500 text-white px-2 py-1 rounded hover:bg-red-600">
        ...
        <nav class="pt-4 text-black">
            ...
            <div class="sidebar-menu text-black">
                ...
                <div class="sidebar-item flex items-center p-3 cursor-pointer hover:bg-gray-100">
                    ...
                    <span class="sidebar-text text-black"><a href="../message/section.html">Section Messages</a></span>
                </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <tbody id="assignmentsBody">
                            ...
                            <?php foreach ($assignments as $assignment): ?>
                                <tr>
                                    ...
                                    <td class="remaining-time" data-last-date="<?= htmlspecialchars($assignment['lastdate']) ?>"></td>
                                    <td>
            ...
            <div class="bg-white p-4 rounded-lg shadow text-black">
                ...
                <form id="messageForm" class="flex items-center space-x-2" method="POST" enctype="multipart/form-data">
                    <input type="hidden" name="noteid" value="<?php echo uniqid(); ?>">
                    <input type="hidden" name="user_id" value="<?php echo $_SESSION['user_id']; ?>"> <!-- Dynamic user ID -->
            ...
            <script>
                ...
                const mockMessages = <?php echo json_encode(array_filter($messages, function($message) {
                    return $message['user_id'] == $_SESSION['user_id'];
                })); ?>;
                ...
                document.getElementById('sendMessageBtn').addEventListener('click', function() {
                    ...
                    if (messageInput.trim() === '' && !document.getElementById('fileInput').files.length) {
